...
// "The 15 verification gates" table — the overview surface a cold maintainer
...
/**
 * The gate names listed in the README handoff's "The 15 verification gates"
...
export function parseReadmeGateNames(readmeText: string): string[] {
  const sectionStart = readmeText.indexOf('### The 15 verification gates');
...
describe('README handoff gate table contract', () => {
  it('has the expected table shape in the handoff section', () => {
    const sectionStart = README.indexOf('### The 15 verification gates');
  ...
  it('lists exactly the runner gate names, in order, with no drift', () => {
    expect(runnerGateNames).toHaveLength(15);
