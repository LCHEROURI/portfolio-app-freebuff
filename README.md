...
| cron-reports | `CRON_SECRET` | deployed cron 401s without auth; daily/weekly/monthly report bodies carry the friendly model heading + raw-id footer; weekly body summarizes deploy failures + rollout-health incidents from the past 7 days (structured `incidentsSummary` field + `## 🚨 Deploy incidents this week` section); monthly also proves the velocity/backlog-drift sections + the AI monthly briefing; no report send envelope |
