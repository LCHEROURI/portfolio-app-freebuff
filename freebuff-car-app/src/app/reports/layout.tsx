...
export default function ReportsLayout({
  ...
  return (
    <div className="min-h-screen bg-ink-50">
      {/* Top bar — mirrors the advisor chrome (back to home + route name). */}
      <header className="sticky top-0 z-30 w-full border-b border-ink-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
        <div className="mx-auto flex h-14 max-w-4xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <span className="text-sm font-semibold text-navy-900">My saved reports</span>
          <Link
            href="/advisor"
            className="rounded-lg px-3 py-1.5 text-sm font-medium text-blue-700 hover:bg-blue-50 hover:text-blue-800 transition-colors"
          >
            New analysis
          </Link>
        </div>
