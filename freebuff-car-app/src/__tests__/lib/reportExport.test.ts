...
describe('side-by-side comparison in exports', () => {
  it('includes a Markdown comparison table with both vehicles specs', () => {
    ...
    // Camry wins both metric rows: MSRP (lower) and MPG (higher) — subtle
    // *(best)* marker, never a value change.
    expect(md).toContain('| MSRP | $28,595 *(best)* | $32,495 |');
    expect(md).toContain('| MPG combined | 33 *(best)* | 29 |');
  ...
  it('includes the same comparison as aligned plain text', () => {
    ...
    expect(txt).toContain('$28,595 (best)');
    expect(txt).toContain('33 (best)');
...
describe('buildReportPlainText', () => {
  ...
  it('marks only the winning column, and only for metric rows', () => {
    const md = buildReportMarkdown(RICH, null);
    const bestCount = (md.match(/\*\(best\)\*/g) || []).length;
    // Exactly two rows have winners here (MSRP, MPG) and Camry wins both.
    expect(bestCount).toBe(2);
    // Non-metric rows never get a marker even when values differ.
    expect(md).toContain('| Drivetrain | FWD | AWD |');
    expect(md).toContain('| Seating | 5 seats | 5 seats |');
  });

  it('marks every tied column and skips all-equal rows', () => {
    const tied: AdvisorState = {
      step: 2,
      vehicles: {
        needs: {},
        comparing: ['a', 'b'],
        names: { a: 'Alpha', b: 'Bravo' },
        specs: {
          a: { title: 'A', msrp: 20000, mpg: 30, seating: 5, drive: 'fwd', safety: 'x' },
          b: { title: 'B', msrp: 20000, mpg: 30, seating: 5, drive: 'awd', safety: 'x' },
        },
      },
    };
    const md = buildReportMarkdown(tied, null);
    // All-equal metric rows have no winner to show; no markers at all.
    expect(md).not.toContain('*(best)*');
  });

  it('marks nothing when a metric is unknown for some columns', () => {
    const partial: AdvisorState = {
      ...
      vehicles: {
        ...
        specs: {
          a: { title: 'A', msrp: 20000, mpg: 30, seating: 5, drive: 'fwd', safety: 'x' },
          b: { title: 'B', msrp: 21000, mpg: null, seating: 5, drive: 'awd', safety: 'x' },
        },
      },
    };
    const md = buildReportMarkdown(partial, null);
    // MPG is unknown for Bravo: MSRP still gets its winner, MPG does not.
    expect(md).toContain('| MSRP | $20,000 *(best)* | $21,000 |');
    expect(md).toContain('| MPG combined | 30 | n/a |');
  });

  it('keeps the md/txt parity contract across the comparison rows', () => {
    const md = buildReportMarkdown(RICH, '2026-09-03T12:00:00Z');
    const txt = buildReportPlainText(RICH, '2026-09-03T12:00:00Z');
    for (const value of ['$28,595', '$32,495', 'FWD', 'AWD']) {
      expect(md).toContain(value);
  ...
  it('matches the Markdown export on every data line', () => {
    ...
    const dataLines = (s: string) =>
      s.split('\n')
        .map((l) => l.replace(/^(#|>|-|\*)+\s*/, '').replace(/\*\*/g, '').replace(/_/g, '').replace(/\|/g, ' ').replace(/\*\(best\)\*/g, '(best)').replace(/\s+/g, ' ').trim().toLowerCase())
