...
export const ConnectionStatusWidget = ({ onClose }: { onClose?: () => void }) => {
  ...
  return (
    <Link
      ...
      title={statuses ? statuses.map((s) => `${s.name}: ${labelOf(s)}`).join(' · ') : text}
      className="block rounded-lg bg-butter-100 px-3 py-2 text-xs transition-colors hover:bg-butter-200 dark:bg-pepper-700 dark:hover:bg-pepper-600"
    >
