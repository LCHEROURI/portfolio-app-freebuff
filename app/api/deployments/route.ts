...
// ============================================================================
// GET /api/deployments — live deployment feed
//
// 1. Vercel API (when VERCEL_TOKEN is set): latest deployment per project.
// 2. Live health checks: every deployment URL is fetched and scored
...
export async function GET(req: NextRequest) {
    ...
    const results = await Promise.allSettled(
      projectNames.map(async (project) => {
        ...
        const environment: Deployment['environment'] =
          ...
          : d.target === 'preview' ? 'preview' : 'preview';
        return {
  ...
  const configured = Boolean(vercelToken) || Boolean(firebaseToken) || checked.length > 0;

  return NextResponse.json({
    ok: true,
    configured,
    source: vercelToken ? 'vercel' : firebaseToken ? 'firebase' : 'none',
    deployments: checked,
  });
}
