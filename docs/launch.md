...
Run **all fourteen in one command** with `npm run verify:all` — it preflights the
...
```text
   ...
   │  runs the 14 verify:all gates + drift guards (timeboxed); a  │
...
runs the complete fourteen-gate suite against production, printing a single
...
5. **Run the full verify suite** against the production URL (§4). All fourteen gates must pass.
